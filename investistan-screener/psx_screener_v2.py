...
def fundamental_score(f: dict) -> tuple[float, list]:
    ...
    roe = f.get("roe")
    if roe is not None:
        if roe > 20:   score += 20; reasons.append(f"ROE {roe:.1f}% (excellent)")
        elif roe > 15: score += 15; reasons.append(f"ROE {roe:.1f}% (good)")
        elif roe > 10: score += 8;  reasons.append(f"ROE {roe:.1f}% (fair)")
        else:          reasons.append(f"ROE {roe:.1f}% (weak)")

    # Leverage -- Debt/Equity
    de = f.get("debt_equity")
    if de is not None:
        if de < 50:    score += 20; reasons.append(f"D/E {de:.0f} (low leverage)")
        elif de < 100: score += 13; reasons.append(f"D/E {de:.0f} (moderate)")
        elif de < 200: score += 5;  reasons.append(f"D/E {de:.0f} (high)")
        else:          reasons.append(f"D/E {de:.0f} (very high leverage)")

    # Valuation -- P/E
    pe = f.get("pe_ratio")
    if pe and pe > 0:
        if pe < 8:     score += 15; reasons.append(f"P/E {pe:.1f} (deep value)")
        elif pe < 12:  score += 12; reasons.append(f"P/E {pe:.1f} (value)")
        elif pe < 18:  score += 8;  reasons.append(f"P/E {pe:.1f} (fair)")
        elif pe < 25:  score += 4;  reasons.append(f"P/E {pe:.1f} (slightly rich)")
        else:          reasons.append(f"P/E {pe:.1f} (expensive)")

    # Dividend
    dy = f.get("dividend_yield")
    if dy is not None and dy > 0:
        if dy > 8:     score += 15; reasons.append(f"Div yield {dy:.1f}% (high income)")
        elif dy > 5:   score += 10; reasons.append(f"Div yield {dy:.1f}% (solid)")
        elif dy > 3:   score += 6;  reasons.append(f"Div yield {dy:.1f}% (moderate)")
        else:          score += 2;  reasons.append(f"Div yield {dy:.1f}% (low)")

    # Net margin
    nm = f.get("net_margin")
    if nm is not None:
        if nm > 20:    score += 10; reasons.append(f"Net margin {nm:.1f}% (excellent)")
        elif nm > 12:  score += 7;  reasons.append(f"Net margin {nm:.1f}% (good)")
        elif nm > 6:   score += 4;  reasons.append(f"Net margin {nm:.1f}% (fair)")
        else:          reasons.append(f"Net margin {nm:.1f}% (thin)")

    # Earnings / revenue growth
    eg = f.get("eps_growth") or f.get("earnings_growth")
    if eg is not None:
        if eg > 20:    score += 10; reasons.append(f"EPS growth {eg:.1f}% (strong)")
        elif eg > 10:  score += 6;  reasons.append(f"EPS growth {eg:.1f}% (solid)")
        elif eg > 0:   score += 3;  reasons.append(f"EPS growth {eg:.1f}% (positive)")
        else:          reasons.append(f"EPS growth {eg:.1f}% (declining)")

    # Stability -- Beta
    beta = f.get("beta")
    if beta is not None:
        if beta < 0.8:   score += 5; reasons.append(f"Beta {beta:.2f} (defensive)")
        elif beta < 1.2: score += 3; reasons.append(f"Beta {beta:.2f} (market-like)")
    ...
    return round(min(score, 100), 1), reasons, data_quality
...
def generate_trade_plan(symbol: str, horizon: str,
    ...
    rr = round((tp2 - price) / (price - stop), 2) if price > stop else 0
...
def run_screener(symbols: list) -> pd.DataFrame:
    ...
    for sym in symbols:
        ...
        f_score, reasons, quality = fundamental_score(f)
