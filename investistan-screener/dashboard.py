...
with st.sidebar:
    ...
    st.markdown(
        ...
        f'box-shadow:0 4px 12px rgba(99,102,241,0.4);font-size:18px">&#127760;</div>'
        f'<div>'
        f'<div style="font-size:16px;font-weight:800;color:#f9fafb;letter-spacing:-0.3px">PSX Screener</div>'
        f'<div style="font-size:10px;color:#374151;font-weight:600;text-transform:uppercase;letter-spacing:0.8px">Pro</div>'
        f'</div></div>'
        # Screener date row
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("➕ Add / Update Holding", expanded=False):
        ...
        if st.button("Add to Portfolio", type="primary"):
            if new_sym:
                add_holding(new_sym, new_qty, new_avg,
        ...
        with st.expander("Details, signals & trade options", expanded=False):
            ...
            if st.session_state.get(f"selling_{sym}"):
                with st.form(key=f"sell_form_{sym}"):
                    ...
                    if submitted:
                        entry = sell_holding(sym, sell_qty, sell_price, sell_notes)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    # Moving averages
    for window, color, dash in [(20,"#ffeb3b","solid"),(50,"#2196f3","solid"),(200,"#ff5722","dot")]:
    ...
    for sp_pt in ew.get("swing_points", []):
        ...
        if idx_pos < len(df):
            ...
            color    = "lime" if sp_pt["type"] == "L" else "red"
            symbol_m = "triangle-up" if sp_pt["type"] == "L" else "triangle-down"
            fig.add_trace(go.Scatter(
                x=[date_val], y=[sp_pt["price"]],
                mode="markers", name=f"EW {sp_pt['type']}",
    ...
    if fib_data.get("Retracements"):
        for label, level in fib_data["Retracements"].items():
            fig.add_hline(y=level, line_dash="dot", line_color="rgba(255,215,0,0.4)",
    ...
    # Volume chart
    vol_colors = ["#26a69a" if close.iloc[i] >= close.iloc[i-1] else "#ef5350" for i in range(len(close))]
    vol_fig = go.Figure(go.Bar(x=df.index, y=volume, marker_color=vol_colors, name="Volume"))
    vol_ma  = volume.rolling(20).mean()
    vol_fig.add_trace(go.Scatter(x=df.index, y=vol_ma, name="Vol MA20",
